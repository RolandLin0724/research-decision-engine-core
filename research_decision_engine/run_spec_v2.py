"""Canonical RunSpec v2 with static random and prior-greedy policy contracts."""
...
class RunSpecV2:
    """Immutable, canonical Core v2 input for one generic workload run.

    Version 2 supports exactly ``random`` and ``greedy_prior``. The latter uses
    only a complete, fixed user-declared prior-utility map. Structural validation
    makes that map truth-free by public contract; it cannot determine whether a
    caller manually copied private benchmark truth into the declared values.
    """

    schema: Literal["rde-core-run-spec/v2"]
    candidates: tuple[CandidateSpec, ...]
    policy_id: Literal["random", "greedy_prior"]
        ...
        *,
        ...
        if type(tie_break) is not str or tie_break != RUNSPEC_CANDIDATE_ORDER:
            raise InvalidPolicyTieBreakError(
                "RunSpec v2 tie_break must be 'runspec_candidate_order'."
            )
        normalized_policy_config, normalized_seed = _normalized_v2_policy(
        ...
        object.__setattr__(self, "schema", RUN_SPEC_V2_SCHEMA)
    ...
    def to_canonical_bytes(self) -> bytes:
        ...
        payload = {
            ...
            "schema": RUN_SPEC_V2_SCHEMA,
            "tie_break": self.tie_break,
        }
        return _canonical_json_bytes(payload)

    def fingerprint(self) -> str:
        """Return SHA-256 of the exact canonical RunSpec v2 bytes."""

        return hashlib.sha256(self.to_canonical_bytes()).hexdigest()

    @classmethod
    def from_canonical_bytes(cls, encoded: bytes) -> RunSpecV2:
        """Decode exact canonical v2 bytes, rejecting aliases and unknown fields."""

        if type(encoded) is not bytes:
            raise TypeError("encoded RunSpec v2 must be exact bytes.")
        try:
            ...
            raise ValueError("RunSpec v2 is not strict UTF-8 JSON.") from exc

        top = _required_object(payload, expected_keys=_RUN_SPEC_V2_KEYS, field_name="RunSpec v2")
        if top["schema"] != RUN_SPEC_V2_SCHEMA:
            raise RunSpecVersionMismatchError(
                "RunSpecV2 requires the exact rde-core-run-spec/v2 schema."
            )

        candidate_payloads = top["candidates"]
        if type(candidate_payloads) is not list:
            raise ValueError("RunSpec v2 candidates must be a JSON array.")
        ...
        try:
            ...
            raise ValueError("RunSpec content violates the v2 contract.") from exc

        if spec.to_canonical_bytes() != encoded:
            raise ValueError("RunSpec v2 bytes are valid JSON but are not canonical.")
        return spec
